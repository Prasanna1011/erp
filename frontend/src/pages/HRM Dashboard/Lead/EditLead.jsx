...
const EditLead = () => {
  ...
  const [leadDataById, setLeadDataById] = useState([]);
  const [leadCV, setLeadCV] = useState([]);

  document.title = "Edit Lead | TechAstha";
  ...
  const validation = useFormik({
    ...
    initialValues: {
      firstname: leadDataById.firstname || "",
      lastname: leadDataById.lastname || "",
      select_gender: leadDataById.select_gender || "",
      education: leadDataById.education || "",
      select_technology: leadDataById.select_technology || "",
      past_company_name: leadDataById.past_company_name || "",
      experience: leadDataById.experience || "",
      email_address: leadDataById.email_address || "",
      contact_number: leadDataById.contact_number || "",
      ctc: leadDataById.ctc || "",
      ectc: leadDataById.ectc || "",
      source: leadDataById.source || "",
      upload_cv: leadDataById.upload_cv || "",
      remarks: leadDataById.remarks || "",
    },
  ...
  const fetchLeadDataByID = async () => {
    try{
      const { data } = await axios.get(`${API_GET_LEAD_BY_ID}${id}/`, config);
      setLeadDataById(data.data[0]);
    }catch(error){
      console.log(error);
    }
                    ...
                    >
                      ...
                      <Row>
                        <Col md={3} className="mb-3">
                          ...
                          <Input
                            type="file"
                            // value={validation.values.upload_cv || ""}
                            className={`form-control ${
                            ...
                            id="upload_cv"
                            name="upload_cv" // Ensure that the name matches the backend's expected field name
                            onChange={(event) =>
