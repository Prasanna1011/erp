...
import { Row, Col, Card, CardBody, Container, Table, Button, Input } from "reactstrap";
import { Breadcrumbs } from "@mui/material";
import GetAuthToken from "TokenImport/GetAuthToken";
import axios from "axios";
import "react-datepicker/dist/react-datepicker.css";
import { API_MISSED_CHECKOUT_DATA, API_GET_CHAT_USERS_LIST, API_GET_USERS } from "Apis/api";
import { DNA } from "react-loader-spinner";

const MissCheckoutData = () => {
  const [missCheckouts, setMissCheckouts] = useState();
  ...
  document.title = "Miss Checkouts | Tech Astha-Portal";

  const config = GetAuthToken();

  const getMissCheckoutData = async () => {
    try {
      setLoading(true)
      const { data } = await axios.get(
        `${API_MISSED_CHECKOUT_DATA}?employee_id=${employeeId}&date=${date}&page=${pageNumber}&page_size=${pageSize}&from_date=${fromDate}&to_date=${toDate}`,
        config
      );
      setMissCheckouts(data.results);
      setLoading(false)
  ...
  useEffect(() => {
    getMissCheckoutData();
    getUsersData()
  }, [employeeId, date, pageNumber, pageSize, nextPage, previousPage, fromDate, toDate]);
          ...
          <Col xl={12}>
            <Card>
              <CardBody className="d-flex align-items-center justify-content-start">
                ...
                <div>
                  <label className="text-bold text-dark">Filter By Employee</label>
                  <Input type="select" onChange={(e) => setEmployeeId(e.target.value)}>
                  ...
                  <Table className="align-middle ">
                    <thead className="table-light">
                      <tr>
                        ...
                        <th className="text-center">Checked-Out at </th>
                    ...
                    <tbody>
                      {missCheckouts &&
                        missCheckouts.map((eachItem, index) => (
                          <tr key={index}>
                            ...
                            <td className="text-center">
                              {eachItem?.total_working_hour === null
                                ? "Yet To be Checked Out"
                                : eachItem?.total_working_hour}
                            </td>
                            <td className="text-center">
                              {eachItem?.misscheckout_reason}
                            </td>
                          </tr>
                        ))}
                    </tbody>
...
export default MissCheckoutData;